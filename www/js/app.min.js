...
//   }, 300);
// }
$rootScope.toggleMain($scope.mainToggle,$scope.mainToggleForce,select)})}}}).directive("menuClick",function($window,$browser){return{restrict:"A",link:function($scope,$element,$attrs){$element.on("click",function(event){if(jQuery(".app-wrap").toggleClass("show-nav"),$attrs.menuClick&&"return-false"==$attrs.menuClick)return event.stopPropagation(),!1})}}}).directive("href",function($rootScope){return{restrict:"A",link:function($scope,$element,$attrs){if($rootScope.externalLinkWindow){var a=new RegExp("/"+window.location.host+"/"),href=void 0!=$attrs.ngHref?$attrs.ngHref:$attrs.href;void 0==href||href.indexOf("http")==-1||a.test(href)||$element.attr("target","_blank")}}}}),angular.module("311App").directive("search",function($rootScope,$state){return{restrict:"E",templateUrl:"views/apps/311App/search.html",link:function($scope,$element,$attrs){var instance="settings.proud_actions_app.instances."+$rootScope.appId;$scope.search=_.get(Proud,instance+".search"),$scope.doSearch=function($e){$state.go("city.search")}}}}),angular.module("311App").directive("pcMap",function($rootScope,$state,Post,NgMap){return{restrict:"E",scope:{center:"@",zoomToIncludeMarkers:"@",zoom:"=",layer:"=",showHome:"=",legend:"@",legendTitle:"@",titleSuffix:"@"},templateUrl:"views/apps/311App/map/map-wrapper.html",link:function($scope,$element,$attrs){function addMarkers(){switch($scope.layer.type){case"wordpress":Post.query({postType:"locations","filter[location-taxonomy]":$scope.layer.slug}).$promise.then(function(data){var icon,marker,markers=[];for(var i in data)marker=data[i],icon=$scope.layer.icon,void 0!=marker.meta&&void 0!=marker.meta.lat&&(markers[i]=new Marker({map:pcMap,position:new google.maps.LatLng(marker.meta.lat,marker.meta.lng),icon:icon,map_icon_label:'<span class="fa pc-map-marker fa-fw fa-3x '+icon+'"></span>'}));// for
$scope.markers=markers});break;case"bicycle":pcLayers.bikeLayer=new google.maps.BicyclingLayer,pcLayers.bikeLayer.setMap(pcMap);break;case"traffic":pcLayers.trafficLayer=new google.maps.TrafficLayer,pcLayers.trafficLayer.setMap(pcMap);break;case"transit":pcLayers.transitLayer=new google.maps.TransitLayer,pcLayers.transitLayer.setMap(pcMap)}}function clearMarkers(){if(void 0!=$scope.markers)for(var i=0;i<$scope.markers.length;i++)$scope.markers[i].setMap(null);for(var i in pcLayers)pcLayers[i].setMap(null);pcLayers={}}var icon={path:MAP_PIN,fillColor:"#00CCBB",fillOpacity:1,strokeColor:"",strokeWeight:0},instance="settings.proud_actions_app.instances."+$rootScope.appId;$scope.zoom=$attrs.zoom?$attrs.zoom:12,$attrs.showHome=void 0!=$attrs.showHome&&null!=$attrs.showHome,
// Grab the center, first from saved location (if we're showing the home marker, then from attrs, lastly from sitewide lat/lng.
$scope.center=$attrs.showHome&&$rootScope.chosenPlace?$rootScope.chosenPlace.geometry.location:$scope.center?$scope.center:$rootScope.location,console.log("chosen plage",$rootScope.chosenPlace),console.log("attrs",$attrs),$scope.layers=_.get(Proud,instance+".map_layers"),console.log("layers",$scope.layers),void 0==$attrs.layer||null==$attrs.layer?$scope.layer=$scope.layers[0]:"random"==$attrs.layer?$scope.layer=$scope.layers[Math.floor(Math.random()*$scope.layers.length)]:$scope.layer=$attrs.layer,$scope.layer=$scope.layers[0];var pcMap,pcLayers={};NgMap.getMap().then(function(map){
// Add home marker
if(pcMap=map,$attrs.showHome&&$rootScope.chosenPlace){var chosenPlace=$rootScope.chosenPlace.geometry.location;console.log("@@@@",chosenPlace),$scope.home=new Marker({map:pcMap,position:new google.maps.LatLng(chosenPlace.lat,chosenPlace.lng),icon:icon,map_icon_label:'<span class="fa pc-map-marker fa-fw fa-3x fa-home"></span>'})}
// Add active layer markers
addMarkers()},1e3),$scope.changeLayer=function(layer){clearMarkers(),$scope.layer=layer,addMarkers()}}}}),angular.module("311App").provider("Menu311",function(){
// Init Proud   
...
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
undoMainToggle:!0},controller:function($scope,$rootScope,$state,$window){$scope.place=null,$scope.chosenPlace=$rootScope.chosenPlace;var instance="settings.proud_actions_app.instances."+$rootScope.appId;_.get(Proud,instance+".background");
//$scope.bgStyle = bgImage ? 'background: url('+ bgImage +');background-size: 100%;min-height: 400px;' : '';
$scope.locate=function(){navigator.geolocation?($rootScope.chosenPlace=null,$state.go("city.local.location",{latlng:position.coords.latitude+","+position.coords.longitude})):alert("Geolocation is not supported by this browser.")},$scope.chosenUpdate=function(){null!=$rootScope.chosenPlace&&($scope.chosenPlace=$rootScope.chosenPlace,$state.go("city.local.location",{latlng:$rootScope.chosenPlace.geometry.location.lat()+","+$rootScope.chosenPlace.geometry.location.lng()}))}}}).state("city.local.location",{url:"/location/:latlng",templateUrl:"views/apps/311App/local/local-location.html",data:{title:"Local",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
...
$scope.services={garbage:"Wednesday",streetSweeping:"First and third Wednesday each month",fire:"Fire District 4",police:"Beat 2"},
...
$scope.showSaveInfo=!1,$scope.toggleSaveInfo=function($e){$e.preventDefault(),$scope.showSaveInfo=!$scope.showSaveInfo}}})}]),angular.module("311App").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("city.faq",{url:"/answers",templateUrl:"views/apps/311App/faq/faq.html",data:{title:"FAQ",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
