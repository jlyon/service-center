...
// Global options
// ---------------------------
$rootScope.activeCity=_.get(Proud,"settings.global.location.city")||"Huntsville",$rootScope.activeState=_.get(Proud,"settings.global.location.state")||"Alabama",$rootScope.location=_.get(Proud,"settings.global.location")||{lat:0,lng:0},$rootScope.mapboxAccessToken=_.get(Proud,"settings.global.mapbox.token")||"",$rootScope.mapboxMap=_.get(Proud,"settings.global.mapbox.map")||"";var chosenPlace=localStorage.getItem("chosenPlace");$rootScope.chosenPlace=chosenPlace?JSON.parse(chosenPlace):null,
...
//   }, 300);
// }
$rootScope.toggleMain($scope.mainToggle,$scope.mainToggleForce,select)})}}}).directive("menuClick",function($window,$browser){return{restrict:"A",link:function($scope,$element,$attrs){$element.on("click",function(event){if(jQuery(".app-wrap").toggleClass("show-nav"),$attrs.menuClick&&"return-false"==$attrs.menuClick)return event.stopPropagation(),!1})}}}).directive("href",function($rootScope){return{restrict:"A",link:function($scope,$element,$attrs){if($rootScope.externalLinkWindow){var a=new RegExp("/"+window.location.host+"/"),href=void 0!=$attrs.ngHref?$attrs.ngHref:$attrs.href;void 0==href||href.indexOf("http")==-1||a.test(href)||$element.attr("target","_blank")}}}}),angular.module("311App").directive("search",function($rootScope,$state){return{restrict:"E",templateUrl:"views/apps/311App/search.html",link:function($scope,$element,$attrs){var instance="settings.proud_actions_app.instances."+$rootScope.appId,searchActive=_.get(Proud,instance+".search");$scope.searchActive=void 0!=searchActive&&searchActive!==!1,$scope.doSearch=function($e){$state.go("city.search")}}}}),angular.module("311App").directive("pcMap",function($rootScope,$state,Post,NgMap){return{restrict:"E",scope:{center:"@",zoomToIncludeMarkers:"@",zoom:"=",layer:"=",showHome:"=",legend:"@",legendTitle:"@",titleSuffix:"@"},templateUrl:"views/apps/311App/map/map-wrapper.html",link:function($scope,$element,$attrs){function addMarkers(){switch($scope.layer.type){case"wordpress":Post.query({postType:"locations","filter[location-taxonomy]":$scope.layer.slug}).$promise.then(function(data){var faIcon,marker,markers=[];for(var i in data)marker=data[i],faIcon=$scope.layer.icon,void 0!=marker.meta&&void 0!=marker.meta.lat&&(markers[i]=new Marker({map:gMap,position:new google.maps.LatLng(marker.meta.lat,marker.meta.lng),icon:{path:MAP_PIN,fillColor:"#00CCBB",fillOpacity:1,strokeColor:"",strokeWeight:0},map_icon_label:'<span class="fa pc-map-marker fa-fw '+faIcon+'"></span>'}));// for
$scope.markers=markers});break;case"bicycle":mapLayers.bikeLayer=new google.maps.BicyclingLayer,mapLayers.bikeLayer.setMap(gMap);break;case"traffic":mapLayers.trafficLayer=new google.maps.TrafficLayer,mapLayers.trafficLayer.setMap(gMap);break;case"transit":mapLayers.transitLayer=new google.maps.TransitLayer,mapLayers.transitLayer.setMap(gMap)}}function clearMarkers(){if(void 0!=$scope.markers)for(var i=0;i<$scope.markers.length;i++)$scope.markers[i].setMap(null);for(var i in mapLayers)mapLayers[i].setMap(null);mapLayers={}}var instance="settings.proud_actions_app.instances."+$rootScope.appId;$scope.mapZoom=$attrs.zoom?$attrs.zoom:12,$attrs.showHome=void 0!=$attrs.showHome&&null!=$attrs.showHome,
// Grab the center, first from saved location (if we're showing the home marker, then from attrs, lastly from sitewide lat/lng.
$scope.center=$attrs.showHome&&$rootScope.chosenPlace?$rootScope.chosenPlace.geometry.location:$scope.center?$scope.center:$rootScope.location,console.log("chosen plage",$rootScope.chosenPlace),console.log("attrs",$attrs),$scope.layers=_.get(Proud,instance+".map_layers"),console.log("layers",$scope.layers),void 0==$attrs.layer||null==$attrs.layer?$scope.layer=$scope.layers[0]:"random"==$attrs.layer?$scope.layer=$scope.layers[Math.floor(Math.random()*$scope.layers.length)]:$scope.layer=$attrs.layer,$scope.layer=$scope.layers[0];var gMap,mapLayers={};NgMap.getMap().then(function(map){
// Add home marker
if(gMap=map,$attrs.showHome&&$rootScope.chosenPlace){var chosenPlace=$rootScope.chosenPlace.geometry.location;$scope.home=new Marker({map:gMap,position:new google.maps.LatLng(chosenPlace.lat,chosenPlace.lng),icon:{path:MAP_PIN,fillColor:"#00CCBB",fillOpacity:1,strokeColor:"#00CCBB",strokeWeight:0},map_icon_label:'<span class="fa fa-fw fa-home"></span>'})}
// Add active layer markers
addMarkers()},1e3),$scope.changeLayer=function(layer){clearMarkers(),$scope.layer=layer,addMarkers()}}}}),angular.module("311App").directive("pcLocate",function($rootScope,$state){return{restrict:"E",scope:{state:"=",requireAddress:"=",place:"@"},templateUrl:"views/apps/311App/pc-locate.html",link:function($scope,$element,$attrs,place){$attrs.requireAddress=void 0!=$attrs.requireAddress&&""==$attrs.requireAddress,$scope.geolocation=navigator.geolocation,$scope.geolocationLoading=!1;var options={types:[],componentRestrictions:{country:"us"}},gPlace=new google.maps.places.Autocomplete($element.find(".input-lg")[0],options);google.maps.event.addListener(gPlace,"place_changed",function(){$rootScope.chosenPlace=gPlace.getPlace(),$scope.chosenUpdate()}),$scope.locate=function(){navigator.geolocation?navigator.geolocation.getCurrentPosition(function(position){if($rootScope.chosenPlace=null,$scope.geolocationLoading=!0,$attrs.requireAddress){var geocoder=new google.maps.Geocoder,latlng={lat:position.coords.latitude,lng:position.coords.longitude};geocoder.geocode({location:latlng},function(results,status){"OK"===status?($rootScope.chosenPlace=results[0],localStorage.getItem("chosenPlace")&&localStorage.setItem("chosenPlace",JSON.stringify($rootScope.chosenPlace)),$scope.geolocationLoading=!1,$state.go($attrs.state,{latlng:position.coords.latitude+","+position.coords.longitude,address:$rootScope.chosenPlace.formatted_address})):alert("Sorry, there was an issue getting an address for your current location. Please try typing your address above.")})}else $scope.geolocationLoading=!1,$state.go($attrs.state,{latlng:position.coords.latitude+","+position.coords.longitude})}):alert("Geolocation is not supported by this browser.")},$scope.chosenUpdate=function(){null!=$rootScope.chosenPlace&&($scope.chosenPlace=$rootScope.chosenPlace,$state.go($attrs.state,{latlng:$rootScope.chosenPlace.geometry.location.lat()+","+$rootScope.chosenPlace.geometry.location.lng(),address:$attrs.requireAddress?$rootScope.chosenPlace.formatted_address:null}))}}}}).directive("pcLocateSave",function($rootScope,$state){return{restrict:"E",/*scope: {
        state: '=',
        requireAddress: '=',
        place: '@'
    },*/
...
$scope.showSaveInfo=!1,$scope.toggleSaveInfo=function($e){$e.preventDefault(),$scope.showSaveInfo=!$scope.showSaveInfo}}}}),angular.module("311App").provider("Menu311",function(){
// Init Proud   
...
title:"Check Status"},templateUrl:"views/apps/311App/search/search.html",controller:function($scope,$rootScope,$state,$window,Search){var instance="settings.proud_actions_app.instances."+$rootScope.appId;$scope.googleSearchSite=_.get(Proud,instance+".search"),$scope.term="",$scope.results=!1,$scope.loading=!1,$scope.autocomplete=function(){$scope.loading=!0,Search.query({term:$scope.term}).$promise.then(function(data){console.log(data),$scope.results=data,$scope.loading=!1})},$scope.go=function(item,$e){if($e.preventDefault(),void 0!=item.action_attr&&""!=item.action_attr){var hash="city/"+item.action_attr+item.action_hash;$window.location.hash=hash}else $window.open(item.url)},$scope.googleSearch=function($e){$e.preventDefault();var url="https://www.google.com/search?q="+encodeURIComponent($scope.term+" site:"+$scope.googleSearchSite);$window.open(url)}}})}]),angular.module("311App").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("city.local",{url:"/local",template:"<div ui-view></div>",data:{title:"Vote",// Sets meta title
...
undoMainToggle:!0},controller:function($scope,$rootScope,$state,$window){
// We just need to determine if we redirect to city.local.address or city.local.location
var redir=function(){if(!$rootScope.chosenPlace){var chosenPlace=localStorage.getItem("chosenPlace");$rootScope.chosenPlace=chosenPlace?JSON.parse(chosenPlace):null}$rootScope.chosenPlace?$state.go("city.local.location",{latlng:$rootScope.chosenPlace.geometry.location.lat+","+$rootScope.chosenPlace.geometry.location.lng}):$state.go("city.local.address")};$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){"city.local"==toState.name&&redir()}),"city.local"==$state.current.name&&redir()}}).state("city.local.address",{url:"/address",templateUrl:"views/apps/311App/local/local-address.html",data:{title:"Vote",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
undoMainToggle:!0},controller:function($scope,$rootScope,$state,$window){}}).state("city.local.location",{url:"/location/:latlng",templateUrl:"views/apps/311App/local/local-location.html",data:{title:"Local",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
undoMainToggle:!0},resolve:{data:function($stateParams,$rootScope,GoogleCivicInfo){if($rootScope.chosenPlace=null,$rootScope.chosenPlace)return $rootScope.chosenPlace;var latlng=$stateParams.latlng.split(",");latlng={lat:parseFloat(latlng[0]),lng:parseFloat(latlng[1])};var deferred=$.Deferred(),geocoder=new google.maps.Geocoder;return geocoder.geocode({location:latlng},function(results,status){"OK"===status?($rootScope.chosenPlace=results[0],localStorage.getItem("chosenPlace")&&localStorage.setItem("chosenPlace",JSON.stringify($rootScope.chosenPlace)),deferred.resolve($rootScope.chosenPlace)):deferred.reject(status)}),deferred.promise()}},controller:function($scope,$rootScope,$state,$window,data){
// Structure the address data
data.address_assoc={};for(var i in data.address_components)data.address_assoc[data.address_components[i].types[0]]=data.address_components[i].long_name;data.address_assoc.administrative_area_level_2=data.address_assoc.administrative_area_level_2.replace("County",""),data.name=void 0!=data.name?data.name:data.formatted_address,$scope.place=data,console.log(data),
// @todo: this is placeholder
$scope.services={garbage:"Wednesday",streetSweeping:"First and third Wednesday each month",fire:"Fire District 4",police:"Beat 2"}}})}]),angular.module("311App").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("city.faq",{url:"/answers",templateUrl:"views/apps/311App/faq/faq.html",data:{title:"FAQ",// Sets meta title
description:"About the about",// Sets different meta description
keywords:"About, this, page",// Sets different meta keywords,
doScroll:!1,// no scroll on route change
...
Post.query({postType:"questions","filter[faq-topic]":$state.params.slug}).$promise.then(function(data){$scope.nodes=data}),
